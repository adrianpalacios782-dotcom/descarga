...
class KickAdapter(BasePlatformAdapter):
    """Adaptador de infraestructura específico para extraer clips y videos de Kick."""

    def detect(self, url: Url) -> bool:
        return url.detect_platform() == "Kick"

    def analyze(self, url: Url) -> MediaMetadata:
        info = self._extract_with_ytdlp(url)
        return self._parse_ytdlp_info(url, info, platform_name="Kick")
